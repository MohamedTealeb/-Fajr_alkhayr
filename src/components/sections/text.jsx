...
const Text = () => {
  ...
  const steps = [
    {
      ...
      desc: 'يختار المتبرع المشروع المناسب من صفحة المشاريع على موقعنا، وتتم عملية الدفع مباشرة، أو يمكنه التواصل معنا للاستفسار والمساعدة في اختيار المشروع الأنسب.'
    },
    {
      icon: <FaPhoneAlt size={32} />, title: 'التواصل مع العميل', num: 2,
      desc: 'من بعد الدفع سيقوم فريقنا بالتواصل مع المتبرع وأخذ منه المعلومات لكتابة السند واللوحة المتواجدة في كل مرحلة وعلى شاهد الأرقام الثابت على البئر.'
    },
    {
      icon: <FaMoneyCheckAlt size={32} />, title: 'تأكيد الطلب', num: 3,
      desc: 'نرسل للمتبرع موقع المشروع المقترح وتصميم اللوحة للاطلاع والموافقة. بعد التأكد على الموقع وصحة المعلومات، نبدأ بتنفيذ المشروع.'
    },
    {
      icon: <FaHammer size={32} />, title: 'بدء التنفيذ', num: 4,
      desc: 'يبدأ التنفيذ مباشرة بعد الموافقة، ويبقى فريقنا على تواصل مستمر مع المتبرع طوال مدة تنفيذ المشروع.'
    },
    {
      icon: <FaRegClipboard size={32} />, title: 'تقرير المشروع', num: 5,
      desc: 'نرسل تقارير يومية تحتوي على صور وفيديوهات لكل مراحل العمل، من البداية حتى الانتهاء. بعد الانتهاء، يتم إرسال عقد الضمان للمتبرع مع الموقع الجغرافي (GPS) للمشروع، كما نقوم بالمتابعة الدورية لضمان استمرارية عمل المشروع.'
    },
  ];
  ...
  return <>
    <section className="w-full py-12 mt-4 px-4 bg-amber-800 flex flex-col items-center justify-center">
      ...
      <a href="https://wa.me/" target="_blank" rel="noopener noreferrer" className="mt-12">
    ...
    <section className="w-full py-16 px-2 md:px-8 bg-[#fafaf7] flex flex-col items-center justify-center">
      ...
      <p className="text-lg md:text-xl text-gray-800 text-center mb-10 max-w-2xl">تجارب عملائنا تضيف ثقة لقرارك، ونحن هنا لمساعدتك في أي استفسار</p>
            ...
            {testimonials.map((img, idx) => (
              <SwiperSlide key={idx}>
                <div className="flex flex-col items-center justify-center">
                  <img src={img} alt={`testimonial-${idx+1}`} className="w-full max-w-md rounded-3xl shadow-xl object-contain" />
                </div>
              </SwiperSlide>
            ))}
          </Swiper>
